...
test('guests can view the sports page for a published meet; unpublished meets 404', function () {
    $meet = Meet::factory()->active()->published()->create();

    $this->get("/meets/{$meet->id}/sports")
        ->assertOk()
        ->assertInertia(fn (AssertableInertia $page) => $page
            ->component('portal/sports')
            ->where('meet.name', $meet->name));

    $hidden = Meet::factory()->active()->create();

    $this->get("/meets/{$hidden->id}/sports")->assertNotFound();
});

test('sports contested in the meet are listed with a real event count each', function () {
    $meet = Meet::factory()->active()->published()->create();
...
test('sports not attached to this meet never appear', function () {
    $meet = Meet::factory()->active()->published()->create();
...
test('sport rows carry no internal or unrelated fields', function () {
    $meet = Meet::factory()->active()->published()->create();
