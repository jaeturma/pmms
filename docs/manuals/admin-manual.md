...
- New accounts are created by the person themselves at the public sign-up
  ...
  and must click a verification link sent to that email before they can
  do anything.
...
- To make someone an Organizer, Delegation Officer, or another
  Administrator, whoever administers the server/database has to change
  their `role` value directly (`docs/authorization.md` documents the
  supported way: `$user->forceFill(['role' => …])->save()` via
  ...
  a new Organizer or Delegation Officer rather than expecting to do it
  from the UI.
...
## 4. Districts / Municipalities registry
...
- `docs/authorization.md` — the full role/action matrix.
- `docs/division.md`, `docs/registry.md`, `docs/sports-catalog.md` — the
  technical reference for everything in §3–6.
