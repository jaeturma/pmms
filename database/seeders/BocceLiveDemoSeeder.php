...
/**
 * Standalone, local-development-only sample dataset for the live bocce
 * scoreboard control feature (end-based point awards toward a configurable
 * target score) — same purpose and conventions as
 * `BilliardLiveDemoSeeder`/`GoalBallLiveDemoSeeder`. Own Meet/Event/
 ...
 *
 * Run with: php artisan db:seed --class=BocceLiveDemoSeeder
 */
class BocceLiveDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $meet = Meet::query()->firstOrCreate(['name' => 'Bocce Scoreboard Demo'], [
            ...
            'venue' => 'Demo Bocce Court',
            'status' => MeetStatus::Active,
        ]);

        $event = Event::query()->firstOrCreate(
            ['sport_id' => $sport->id, 'name' => 'Bocce Boys Secondary Singles (Demo)'],
            [
        ...
        [$schoolA, $entryA] = $this->player($meet, $district, $event, 'Pallino Demo High School', 'Enzo', 'Aquino');
        [$schoolB, $entryB] = $this->player($meet, $district, $event, 'Volo Demo High School', 'Dario', 'Mercado');
        ...
        $this->command?->info("Bocce scoreboard demo ready — /matches/{$match->id}/scoreboard");
    }
    ...
    private function scriptSession(EventMatch $match, School $schoolA, School $schoolB, User $admin): void
    {
        ...
        $state = [
            'ends' => [
                ['end' => 1, 'winner' => 'a', 'points' => 2],
                ['end' => 2, 'winner' => 'b', 'points' => 1],
                ['end' => 3, 'winner' => 'a', 'points' => 3],
            ],
            'ends_played' => 3,
            'target_score' => 12,
        ];
        ...
        $log = function (ScoreEventType $type, array $payload = []) use (&$t, $session, $admin) {
            $t = $t->copy()->addSeconds(random_int(120, 240));
        ...
        $log(ScoreEventType::EndComplete, ['end' => 1, 'winner' => 'a', 'points' => 2, 'score_a' => 2, 'score_b' => 0]);
        $log(ScoreEventType::EndComplete, ['end' => 2, 'winner' => 'b', 'points' => 1, 'score_a' => 2, 'score_b' => 1]);
        $log(ScoreEventType::EndComplete, ['end' => 3, 'winner' => 'a', 'points' => 3, 'score_a' => 5, 'score_b' => 1]);
