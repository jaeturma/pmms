...
/**
 * Standalone, local-development-only sample dataset for the live softball/
 * baseball scoreboard control feature (at-bat indicator, count/outs,
 * inning runs) — same purpose and conventions as
 * `BasketballLiveDemoSeeder`/`BoxingLiveDemoSeeder`. Own Meet/Event/
 ...
 *
 * Run with: php artisan db:seed --class=SoftballLiveDemoSeeder
 */
class SoftballLiveDemoSeeder extends Seeder
{
    public function run(): void
    {
        $sport = Sport::query()->firstOrCreate(['name' => 'Softball'], ['active' => true]);

        $meet = Meet::query()->firstOrCreate(['name' => 'Softball Scoreboard Demo'], [
        ...
        $event = Event::query()->firstOrCreate(
            ['sport_id' => $sport->id, 'name' => 'Softball Girls Secondary (Demo)'],
            [
        ...
        [$schoolA, $entryA] = $this->captain($meet, $district, $event, 'Comets Demo High School', 'Away', 'Captain');
        [$schoolB, $entryB] = $this->captain($meet, $district, $event, 'Stars Demo High School', 'Home', 'Captain');
        ...
        $this->command?->info("Softball scoreboard demo ready — /matches/{$match->id}/scoreboard");
    }
    ...
    private function scriptSession(EventMatch $match, School $schoolA, School $schoolB, User $admin): void
    {
        ...
        $state = [
            'inning' => 3, 'half' => 'bottom', 'outs' => 1, 'balls' => 2, 'strikes' => 1,
            'innings' => [
                ['inning' => 1, 'runs_a' => 1, 'runs_b' => 0],
                ['inning' => 2, 'runs_a' => 0, 'runs_b' => 2],
            ],
            'innings_scheduled' => 7,
        ...
        $log(ScoreEventType::InningRun, ['side' => 'a', 'runs' => 1, 'inning' => 1]);
        $log(ScoreEventType::Count, ['action' => 'out', 'outs' => 1]);
        $log(ScoreEventType::Count, ['action' => 'out', 'outs' => 2]);
        $log(ScoreEventType::Count, ['action' => 'out', 'outs' => 0]);
        $log(ScoreEventType::InningRun, ['side' => 'b', 'runs' => 2, 'inning' => 2]);
        $log(ScoreEventType::Count, ['action' => 'ball', 'balls' => 1, 'strikes' => 0]);
        $log(ScoreEventType::Count, ['action' => 'strike', 'balls' => 1, 'strikes' => 1]);
        $log(ScoreEventType::Count, ['action' => 'ball', 'balls' => 2, 'strikes' => 1]);
