 ...
 * rounds scoreboard feature as seen through Taekwondo (round/rest clock,
 * bell, judged rounds) — proves `ScoreboardType::CombatRounds` resolves
 * and renders correctly for a real sport name, reusing the exact same
 * `BoxingGameControl` component and round()/roundClock()/bell()/
 * settings() endpoints boxing already has (see the controller's own
 * docblocks for why the two board types are treated identically). Same
 * purpose and conventions as `BoxingLiveDemoSeeder`. Own Meet/Event/
 ...
 * re-run.
 *
 * Run with: php artisan db:seed --class=TaekwondoLiveDemoSeeder
 */
class TaekwondoLiveDemoSeeder extends Seeder
{
    public function run(): void
    {
        $sport = Sport::query()->firstOrCreate(['name' => 'Taekwondo'], ['active' => true]);

        $meet = Meet::query()->firstOrCreate(['name' => 'Taekwondo Scoreboard Demo'], [
        ...
        $event = Event::query()->firstOrCreate(
            ['sport_id' => $sport->id, 'name' => 'Taekwondo Boys Secondary -55kg (Demo)'],
            [
        ...
        [$schoolA, $entryA] = $this->athlete($meet, $district, $event, 'Hong Demo High School', 'Jomari', 'Cruz');
        [$schoolB, $entryB] = $this->athlete($meet, $district, $event, 'Chung Demo High School', 'Enzo', 'Villanueva');
        ...
        $this->command?->info("Taekwondo scoreboard demo ready — /matches/{$match->id}/scoreboard");
    }
    ...
    private function scriptSession(EventMatch $match, School $schoolA, School $schoolB, User $admin): void
    {
        ...
        $log(ScoreEventType::RoundScore, ['round' => 1, 'score_a' => 7, 'score_b' => 5]);
        $log(ScoreEventType::Bell);
