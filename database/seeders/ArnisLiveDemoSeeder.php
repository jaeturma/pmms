...
/**
 * Standalone, local-development-only sample dataset for the live combat-
 * rounds scoreboard feature as seen through Arnis (round/rest clock,
 * bell, judged rounds) — proves `ScoreboardType::CombatRounds` resolves
 * for Arnis too (added after Taekwondo/Wushu/Pencak Silat, same real
 * round-clock/red-blue-corner shape), reusing the exact same
 * `BoxingGameControl` component and round()/roundClock()/bell()/
 * settings() endpoints boxing already has. Same purpose and conventions
 * as `TaekwondoLiveDemoSeeder`. Own Meet/Event/Schools/Delegations/
 * athletes, doesn't touch or depend on any other seeder's dataset.
 * Idempotent throughout (`firstOrCreate`), safe to re-run.
 *
 * Run with: php artisan db:seed --class=ArnisLiveDemoSeeder
 */
class ArnisLiveDemoSeeder extends Seeder
{
    public function run(): void
    {
        $sport = Sport::query()->firstOrCreate(['name' => 'Arnis'], ['active' => true]);

        $meet = Meet::query()->firstOrCreate(['name' => 'Arnis Scoreboard Demo'], [
        ...
        $event = Event::query()->firstOrCreate(
            ['sport_id' => $sport->id, 'name' => 'Arnis Anyo/Labanan Boys Secondary (Demo)'],
            [
        ...
        [$schoolA, $entryA] = $this->athlete($meet, $district, $event, 'Kali Demo High School', 'Jericho', 'Bautista');
        [$schoolB, $entryB] = $this->athlete($meet, $district, $event, 'Eskrima Demo High School', 'Marlon', 'Santiago');
        ...
        $this->command?->info("Arnis scoreboard demo ready — /matches/{$match->id}/scoreboard");
    }
    ...
    private function scriptSession(EventMatch $match, School $schoolA, School $schoolB, User $admin): void
    {
        ...
        $log(ScoreEventType::RoundScore, ['round' => 1, 'score_a' => 8, 'score_b' => 6]);
        $log(ScoreEventType::Bell);
