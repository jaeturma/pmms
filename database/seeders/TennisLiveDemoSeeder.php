...
/**
 * Standalone, local-development-only sample dataset for the live tennis
 * scoreboard control feature (deuce/tiebreak game-set-match state
 * machine, single-level undo) — same purpose and conventions as
 * `TableTennisLiveDemoSeeder`/`WrestlingLiveDemoSeeder`. Own Meet/Event/
 ...
 *
 * Run with: php artisan db:seed --class=TennisLiveDemoSeeder
 */
class TennisLiveDemoSeeder extends Seeder
{
    public function run(): void
    {
        $sport = Sport::query()->firstOrCreate(['name' => 'Tennis'], ['active' => true]);

        $meet = Meet::query()->firstOrCreate(['name' => 'Tennis Scoreboard Demo'], [
        ...
        $event = Event::query()->firstOrCreate(
            ['sport_id' => $sport->id, 'name' => 'Tennis Boys Secondary Singles (Demo)'],
            [
        ...
        [$schoolA, $entryA] = $this->player($meet, $district, $event, 'Baseline Demo High School', 'Rafael', 'Cruz');
        [$schoolB, $entryB] = $this->player($meet, $district, $event, 'Volley Demo High School', 'Andres', 'Reyes');
        ...
        $this->command?->info("Tennis scoreboard demo ready — /matches/{$match->id}/scoreboard");
    }
    ...
    private function scriptSession(EventMatch $match, School $schoolA, School $schoolB, User $admin): void
    {
        ...
        $state = [
            'sets' => [
                ['set' => 1, 'score_a' => 6, 'score_b' => 4],
            ],
            'sets_won_a' => 1, 'sets_won_b' => 0,
            'current_set_games_a' => 3, 'current_set_games_b' => 2,
            'current_game_points_a' => 3, 'current_game_points_b' => 2,
            'is_tiebreak' => false,
        ...
        $log(ScoreEventType::SetComplete, ['set' => 1, 'score_a' => 6, 'score_b' => 4, 'sets_won_a' => 1, 'sets_won_b' => 0]);
        $log(ScoreEventType::Possession, ['side' => 'a']);
        $log(ScoreEventType::TennisPoint, ['side' => 'a', 'current_set_games_a' => 3, 'current_set_games_b' => 2, 'current_game_points_a' => 1, 'current_game_points_b' => 2, 'is_tiebreak' => false]);
        $log(ScoreEventType::TennisPoint, ['side' => 'a', 'current_set_games_a' => 3, 'current_set_games_b' => 2, 'current_game_points_a' => 2, 'current_game_points_b' => 2, 'is_tiebreak' => false]);
        $log(ScoreEventType::TennisPoint, ['side' => 'a', 'current_set_games_a' => 3, 'current_set_games_b' => 2, 'current_game_points_a' => 3, 'current_game_points_b' => 2, 'is_tiebreak' => false]);
