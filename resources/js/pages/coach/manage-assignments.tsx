...
export default function ManageCoachAssignments({
    registration,
    events,
    selectedEventIds,
    canApprove,
        ...
        form.setData(
            ...
            checked
                ...
                : form.data.event_ids.filter(
                      (eventId) => !sportEventIds.includes(eventId),
                  ),
        );
    };
    ...
    return (
        <div className="space-y-6 p-4">
            ...
            <PageHeader
                ...
                description={`${registration.meet} · Event assignment`}
                actions={
                    <Button variant="outline" asChild>
                        <Link href="/coach/assignment-requests">
                            <ArrowLeft />
                            Back
                        </Link>
                    </Button>
            ...
            <section className="grid gap-5 rounded-xl border p-5 md:grid-cols-[9rem_1fr]">
                <div className="flex size-36 items-center justify-center overflow-hidden rounded-xl border bg-muted text-2xl font-semibold">
                    {registration.profile_url ? (
                        <img
                            src={registration.profile_url}
                            alt={`${registration.coach} profile`}
                ...
                <div className="space-y-4">
                    ...
                    <dl className="grid gap-3 text-sm sm:grid-cols-2">
                        <div>
                            ...
                            <dd className="font-medium">
                                {registration.team ?? 'Not assigned'}
                            </dd>
                        </div>
                        <div>
                    ...
                    {registration.certification_url ? (
                        <Button
                            type="button"
                            variant="outline"
                        ...
                        <p className="text-sm text-muted-foreground">
                            No coaching certificate uploaded (optional).
                        </p>
                    )}
                </div>
            </section>
            ...
            >
                <button
                    ...
                    className={`rounded-md px-4 py-2 text-sm font-medium ${activeTab === 'events' ? 'bg-background shadow-sm' : 'text-muted-foreground'}`}
                ...
                <button
                    ...
                    className={`rounded-md px-4 py-2 text-sm font-medium ${activeTab === 'athletes' ? 'bg-background shadow-sm' : 'text-muted-foreground'}`}
                    onClick={() => setActiveTab('athletes')}
                >
                    Registered Athletes (
                    {registration.registered_athletes.length})
                </button>
            </div>
            {activeTab === 'events' ? (
                <form
                    ...
                    onSubmit={(event) => {
                        ...
                        form.put(
                            `/coach/onboarding-requests/${registration.id}/assignments`,
                            { preserveScroll: true },
                        );
                    }}
                >
                    <div>
                        <h2 className="font-semibold">Available events</h2>
                        <p className="text-sm text-muted-foreground">
                            Select one or more events. Saving assignments does
                            not approve the coach account; ICT approval is
                            separate.
                        </p>
                    </div>
                    <div className="space-y-6">
                        {Object.entries(eventsBySport).map(
                            ([sport, sportEvents]) => {
                                const allSelected = sportEvents.every((event) =>
                                    form.data.event_ids.includes(event.id),
                                );
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {canApprove && registration.status !== 'approved' && (
                            <span className="text-sm text-muted-foreground">
                                Return to Coaches after saving to approve the
                                account.
                            </span>
                        )}
                    </div>
                ...
                <section className="space-y-3">
                    <div>
                        <h2 className="font-semibold">Registered Athletes</h2>
                        <p className="text-sm text-muted-foreground">
                            Athletes submitted by this coach under the assigned
                            delegation.
                        </p>
                    </div>
                    {registration.registered_athletes.length === 0 ? (
                        <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">
                            No athletes registered by this coach.
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
                            {registration.registered_athletes.map((athlete) => (
                                <a
                                    key={athlete.id}
                                    href={athlete.profile_url}
                                    className="flex items-center gap-3 rounded-xl border p-4 hover:bg-muted/50"
                                >
                                    <div className="flex size-14 shrink-0 items-center justify-center overflow-hidden rounded-full border bg-muted font-semibold">
                                        {athlete.photo_url ? (
                                            <img
            ...
            <Dialog open={showCertificate} onOpenChange={setShowCertificate}>
                <DialogContent className="flex h-[90vh] max-w-5xl flex-col">
                    <DialogHeader>
                        <DialogTitle>
                            {registration.coach} — Coaching Certification
                        </DialogTitle>
                    </DialogHeader>
                    {registration.certification_url &&
                        (registration.certification_mime_type ===
                        'application/pdf' ? (
                            <iframe
                                src={registration.certification_url}
