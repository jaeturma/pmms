...
import { readCount, readHitsAndErrors, readRunners } from '@/apps/portal/lib/softball-state';
...
function CountDots({ count, max, tone }: { count: number; max: number; tone: 'accent' | 'maroon' | 'live' }) {
    const dots = Array.from({ length: max }, (_, index) => index < count);
    const onClass = { accent: 'bg-[var(--portal-accent)]', maroon: 'bg-[var(--portal-maroon)]', live: 'bg-[var(--portal-live)]' }[tone];
        ...
        <div className="flex justify-center gap-2">
            {dots.map((on, index) => (
                <span
                    key={index}
                    aria-hidden="true"
                    className={cn('size-[19px] rounded-full border-2', on ? cn(onClass, 'border-transparent') : 'border-[var(--portal-border)]')}
                />
            ))}
        </div>
    );
...
function PlayRow({ play, highlight }: { play: PortalPlayByPlayEntry; highlight: boolean }) {
    return (
        ...
        >
            <span className="text-xs font-[850] sm:text-sm">{play.created_at}</span>
...
export function PortalSoftballScoreboard({ liveNow, className }: PortalSoftballScoreboardProps) {
                ...
                <div className="px-3 py-4 sm:px-8 sm:py-4">
                    <div className="grid grid-cols-[1fr_105px_1fr] items-center gap-2 px-1 py-1 sm:grid-cols-[1fr_200px_1fr] sm:gap-7 sm:px-8">
                        <div className="flex flex-col items-center gap-2 text-center sm:grid sm:grid-cols-[120px_1fr] sm:items-center sm:gap-[18px] sm:text-left">
                            ...
                            <div>
                                ...
                                <p className="mt-2 text-[13px] font-[950] uppercase sm:text-[23px]">{session.side_a_label ?? 'TBD'}</p>
                            </div>
                        </div>
                        ...
                        <div className="overflow-hidden rounded-[9px] border border-[var(--portal-border)] bg-[var(--portal-surface)] text-center">
                            ...
                            <div
                                ...
                                aria-label={`Runners on base: ${
                                    [runners?.first && 'first', runners?.second && 'second', runners?.third && 'third'].filter(Boolean).join(', ') ||
                                    'none'
                                }`}
                            >
                                <span
                                    ...
                                    className={cn(
                                        ...
                                        runners?.second ? 'bg-[var(--portal-accent)]' : 'bg-[var(--portal-border)]',
                                    )}
                                />
                                <span
                                    aria-hidden="true"
                                    className={cn(
                                        ...
                                        runners?.first ? 'bg-[var(--portal-accent)]' : 'bg-[var(--portal-border)]',
                                    )}
                                />
                                <span
                                    aria-hidden="true"
                                    className={cn(
                                        ...
                                        runners?.third ? 'bg-[var(--portal-accent)]' : 'bg-[var(--portal-border)]',
                                    )}
                                />
                            </div>
                            <div className="px-1 pb-2.5 text-xs font-[850] sm:pb-3 sm:text-[17px]">
                                ...
                                |<span className="px-2">{count.outs === 1 ? '1 OUT' : `${count.outs} OUTS`}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-2 text-center sm:grid sm:grid-cols-[1fr_120px] sm:items-center sm:gap-[18px] sm:text-right">
                            ...
                            <div className="sm:order-1">
                                ...
                                <p className="mt-2 text-[13px] font-[950] uppercase sm:text-[23px]">{session.side_b_label ?? 'TBD'}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mx-auto mt-2 grid w-full max-w-[460px] grid-cols-3 overflow-hidden rounded-[9px] border border-[var(--portal-border)] sm:mt-2.5">
                        <div className="border-r border-[var(--portal-border)] px-3 py-2 text-center">
                            <div className="mb-1.5 text-xs font-[900] text-[var(--portal-fg)]">BALLS</div>
                            <CountDots count={count.balls} max={3} tone="accent" />
                        </div>
                        <div className="border-r border-[var(--portal-border)] px-3 py-2 text-center">
                            <div className="mb-1.5 text-xs font-[900] text-[var(--portal-fg)]">STRIKES</div>
                            <CountDots count={count.strikes} max={3} tone="maroon" />
                        </div>
                        <div className="px-3 py-2 text-center">
                            <div className="mb-1.5 text-xs font-[900] text-[var(--portal-fg)]">OUTS</div>
                    ...
                    <table className="w-full border-collapse overflow-hidden rounded-[9px] border border-[var(--portal-border)] text-[13px]">
                        <thead>
                            <tr className="bg-[var(--portal-ink)] text-[12px] text-[var(--portal-ink-foreground)]">
                                <th className="border-r border-b border-[var(--portal-border)] py-2 pl-8 text-left font-semibold">Team</th>
                                {INNING_COLUMNS.map((n) => (
                                    <th key={n} className="border-r border-b border-[var(--portal-border)] py-2 font-semibold">
                                        {n}
                                    </th>
                                ))}
                                <th className="border-r border-b border-[var(--portal-border)] py-2 font-semibold">R</th>
                                <th className="border-r border-b border-[var(--portal-border)] py-2 font-semibold">H</th>
                                <th className="border-b border-[var(--portal-border)] py-2 font-semibold">E</th>
                            </tr>
                        </thead>
                        <tbody>
                            {(
                                [
                                    { label: session.side_a_label ?? 'TBD', runs: session.score_a ?? 0, hits: hitsAndErrors?.hits_a, errors: hitsAndErrors?.errors_a, side: 'a' as const },
                                    { label: session.side_b_label ?? 'TBD', runs: session.score_b ?? 0, hits: hitsAndErrors?.hits_b, errors: hitsAndErrors?.errors_b, side: 'b' as const },
                                ]
                            ).map((row) => (
                                <tr key={row.side}>
                                    <td className="border-r border-b border-[var(--portal-border)] py-2 pl-8 text-left font-[850]">{row.label}</td>
                                    {INNING_COLUMNS.map((n) => {
                                        const inningRow = count.innings.find((i) => i.inning === n);
                                        const value = inningRow ? (row.side === 'a' ? inningRow.runs_a : inningRow.runs_b) : null;

                                        return (
                                            <td key={n} className="border-r border-b border-[var(--portal-border)] py-2 text-center">
                                                {value ?? '–'}
                                            </td>
                                        );
                                    })}
                                    <td className="border-r border-b border-[var(--portal-border)] py-2 text-center font-[900]">{row.runs}</td>
                                    <td className="border-r border-b border-[var(--portal-border)] py-2 text-center font-[900]">{row.hits ?? '–'}</td>
                                    <td className="border-b border-[var(--portal-border)] py-2 text-center font-[900]">{row.errors ?? '–'}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
                <div className="pt-2 pb-3 text-center text-xs text-[var(--portal-muted-foreground)]">R - Runs &nbsp;&nbsp;|&nbsp;&nbsp; H - Hits &nbsp;&nbsp;|&nbsp;&nbsp; E - Errors</div>
