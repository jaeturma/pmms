...
import type { PortalMeetSummary, PortalSportCard as PortalSportCardData } from '@/apps/portal/types';
...
export default function PortalSportsDirectory({ meet, sports }: Props) {
    const [classification, setClassification] = useState<Classification>('regular');
    const [query, setQuery] = useState('');

    const regularCount = useMemo(() => sports.filter((sport) => !sport.is_paragames).length, [sports]);
    const paragamesCount = useMemo(() => sports.filter((sport) => sport.is_paragames).length, [sports]);

    const filteredSports = useMemo(() => {
        const term = query.trim().toLowerCase();
        const byClassification = sports.filter((sport) => (classification === 'paragames' ? sport.is_paragames : !sport.is_paragames));

        return term === '' ? byClassification : byClassification.filter((sport) => sport.name.toLowerCase().includes(term));
        ...
        <>
            ...
            <div className="flex flex-col gap-6">
                <PortalHero
                    title="Sports"
                    description={
                        meet ? `Browse every sport contested at ${meet.name} and beyond.` : 'Browse the full provincial meet sports catalog.'
                    }
                />

                {sports.length === 0 ? (
                    <PortalEmptyState icon={Trophy} title="No sports available yet" />
                ) : (
                    <>
                        <PortalSectionHeader
                            title="All Sports"
                        ...
                        <PortalTabs
                            tabs={[
                                { value: 'regular', label: `Regular Sports (${regularCount})` },
                                { value: 'paragames', label: `Paragames (${paragamesCount})` },
                            ]}
                        ...
                        {filteredSports.length === 0 ? (
                            <PortalEmptyState icon={Search} title={`No sport matches "${query}".`} />
                        ) : (
                            <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                {filteredSports.map((sport) => (
                                    <PortalSportCard key={sport.id} sport={sport} />
                                ))}
                            </section>
                        )}
                    </>
