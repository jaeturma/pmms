...
class RegistrationDataConsistencyService
{
    ...
    {
        return $this->athleteIssues()->concat($this->coachIssues())->concat($this->gameIssues())->values();
    }

    /** @return Collection<int, array<string, mixed>> */
    private function athleteIssues(): Collection
    {
        return Athlete::query()->with(['delegation.meet', 'delegation.school', 'delegation.district', 'school.district', 'school.schoolDistrict', 'coaches'])
    ...
    /** @return Collection<int, array<string, mixed>> */
    private function coachIssues(): Collection
    {
        ...
        $unlinkedAccounts = User::query()->where('role', UserRole::Coach->value)->whereDoesntHave('personnel')->get()
    ...
    /** @return Collection<int, array<string, mixed>> */
    private function gameIssues(): Collection
    {
        $entries = Entry::query()->with(['athlete.delegation', 'event', 'delegation'])->get()
        ...
        $members = TeamEntryMember::query()->with(['entry', 'teamEntry'])->get()
        ...
        $matches = EventMatch::query()->withCount(['entries', 'teamEntries'])->with('event')->get()
