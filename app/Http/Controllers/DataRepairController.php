...
class DataRepairController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $issues = $this->consistency->issues($this->sportScope($request));
    ...
    public function repair(Request $request): RedirectResponse
    {
        ...
        abort_unless($this->consistency->issues($this->sportScope($request))->contains(
            fn (array $issue): bool => $issue['type'] === $data['type']
                && $issue['id'] === $data['id'] && ($issue['repair']['code'] ?? null) === $data['code'],
        ), 403);
    ...
    private function sportScope(Request $request): ?Collection
    {
        $user = $request->user();

        return $user->isAdmin() || $user->canManageProductionAccounts()
            ? null
            : $user->tournamentAthleteSportIds();
    }
}
