...
class AthleteReadinessController extends Controller
{
    public function __invoke(Request $request): Response
    {
        ...
        abort_unless($user->hasRole(UserRole::Admin, UserRole::Organizer)
            || $user->hasPermission(Permission::AthleteEligibilityReview, $meet)
            || $user->hasPermission(Permission::DistrictReadinessView, $meet)
            || $user->hasPermission(Permission::MunicipalityReadinessView, $meet), 403);
        ...
        $athletes = $query->get()->map(function (Athlete $athlete) use ($required): array {
            ...
            return ['id' => $athlete->id, 'name' => $athlete->fullName(), 'school' => $athlete->school->name,
                ...
                'medical_cleared' => $medicalCleared, 'accredited' => $athlete->accreditation !== null,
        ...
        return Inertia::render('readiness/index', [
            ...
            'summary' => ['athletes' => $athletes->count(), 'documents_complete' => $athletes->where('documents_complete', true)->count(),
                'dsac_approved' => $athletes->where('dsac_approved', true)->count(), 'medical_cleared' => $athletes->where('medical_cleared', true)->count(),
                'accredited' => $athletes->where('accredited', true)->count(), 'needs_attention' => $athletes->where('needs_attention', true)->count()],
            'districts' => $athletes->groupBy('school_district')->map(fn ($rows, $name) => ['name' => $name, 'athletes' => $rows->count(),
                'ready' => $rows->where('needs_attention', false)->count(), 'needs_attention' => $rows->where('needs_attention', true)->count()])->values(),
